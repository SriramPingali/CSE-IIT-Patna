...
// going through further page requests
int findIndex(int frame[], int cap, vector<int> pgnums, int cur) {
    ...
    for(int i = 0; i < cap; i++) {
        ...
        for(int j = cur+1; j < pgnums.size(); j++) {
...
int main() {
    ...
    while(finput) {
        ...
        for(int i = initialIndex; i < pgnums.size(); i++) {
            ...
            // replace the page which won't be requested in the near future with next page
            int nextPage = pgnums[i];
