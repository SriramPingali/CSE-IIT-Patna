...
def get_words_tags_bigram(data):
    all_words, all_tags = [], []
    temp_words, temp_tags = [START_SYMBOL], [START_SYMBOL]
...
def get_transitions_probs(ner_tags):
    ...
    for bigram in bigram_count:
        transition_values[bigram] = math.log(bigram_count[bigram], 2) - math.log(unigram_count[bigram[0]],2)
...
def transitions_output(transition_values, filename):
    ...
    bigrams = transition_values.keys()
    bigrams = sorted(bigrams)  
    for bigram in bigrams:
        output = " ".join(['BIGRAM', bigram[0], bigram[1], str(transition_values[bigram])])
...
def viterbi_decode(ner_dev_words, taglist, known_words, transition_values, e_values):
    ...
    for item in ner_dev_words:
        ...
        cur_len = 0
        Path_Pre = {} 
        Path_Pre[START_SYMBOL] = [START_SYMBOL]
        Unigram_Pre = [START_SYMBOL]           

        Pi_Pre = {}
        Pi_Pre[START_SYMBOL] = 0
            ...
            for v in tagspace:
                ...
                for w in taglist:
                    ...
                    if w not in Unigram_Pre:
                        continue
                    bigram_cur = (w, v)
                    if bigram_cur not in transition_values:
                        transition_values[bigram_cur] = LOG_PROB_OF_ZERO
                    if v not in Pi_Cur:
                        Pi_Cur[v] = Pi_Pre[w]+transition_values[bigram_cur]+e_values[emi_tmp]
                        w_tmp = w
                    elif Pi_Pre[w]+transition_values[bigram_cur]+e_values[emi_tmp] > Pi_Cur[v]:
                        Pi_Cur[v] = Pi_Pre[w]+transition_values[bigram_cur]+e_values[emi_tmp]
                        w_tmp = w
                if w_tmp != '':
...
OUTPUT_PATH = 'op_bigram_with_context/'
EXTRA_PATH = 'op_bigram_with_context/probabilities/'
...
def print_accuracy(test_ner_tags, test_tags, suffix):
    ...
    for orig_tag in test_ner_tags:
        flat_orig_tags.extend(orig_tag[1:-1])

    # Flattening all original tags
    flat_predict_tags = []
    for predict_tag in test_tags:
        flat_predict_tags.extend(predict_tag[1:-1])
...
def main():
    ...
    train_words, train_tags = get_words_tags_bigram(train_data)
    val_words, val_tags = get_words_tags_bigram(val_data)
    test_words, test_tags = get_words_tags_bigram(test_data)
