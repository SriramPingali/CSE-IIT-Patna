...
class hill_climbing:
    ...
    def solve_eight_puzzle(self, current_state, heuristic_choice,depth = 0):
        stack = [current_state]
        print("\nSolving the puzzle...")
        print("Output being printed to hill_climbing_output.txt...\n")
        while len(stack) != 0 :
            ...
            if current_state == self.final_configuration: # We got the result!
                ...
                out = output_result(self.initial_configuration, "hill_climbing_output.txt", state_parent,
                                    self.state_explored)
...
def startSearching(startState, goalState, file):
    ...
    file = open("hill_climbing_output.txt", "a")
    if status == 1:
...
if __name__ == '__main__':
    ...
    file = open("hill_climbing_output.txt", "w+")
    ...
    goalState = goalState.replace("T", "").replace("B", "0")
