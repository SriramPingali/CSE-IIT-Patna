...
"""Directory containing input files StartState.txt and GoalState.txt"""
d = os.getcwd()
...
def a_star(start_state, h_fun):
    ...
    while pq:
        ...
        for neighbor in neighbors:
            ...
            neighbor.hval = neighbor.cost + h_fun(neighbor.state)
...
def valid_move(state, position):
    ...
    if position == 'UP': 
        if index not in range(0, side_puzzle):           
            temp = new_state[index - side_puzzle]
            new_state[index - side_puzzle] = new_state[index]
...
def output(frontier, time, heuristic_name):
    filename=d+os.sep+'Astar_Output_8Puzzle'+'_'+heuristic_name+'.txt'
    if frontier:
        global moves
        global optimal_path
        moves = backtrack()
        ...
        with open(filename, 'w') as f:
            ...
            f.write("\nOptimal Path : " + str(optimal_path))
            f.write("\nOptimal Path Cost: " + str(len(moves)))
            f.write("\Time taken from execution: " + format(time, '.8f'))
            f.close()
            print("\nExport complete.")
    else:
        with open(filename, 'w') as f:
            ...
            f.write("\nStart State : "+str(start_state))
            f.write("\nGoal State : "+str(goal_state))
            f.write("\nTotal no. of states explored before termination : " + str(count_explored_nodes))
            f.write("\nTotal no. of states to reach the optimal path : " + str(len(moves)+1))
            f.write("\nDepth searched before termination: " + str(max_depth_reached))
            f.write("\Time taken from execution: " + format(time, '.8f'))
            f.close()
            print("\nExport complete.")
